...
const _getMarcaPorVeiculo = () => {
  return 'select marca,count(marca) as quantidade from veiculo group by marca order by quantidade desc'
}

const _getModeloPorVeiculo = () => {
  return 'select modelo,count(modelo) as quantidade from veiculo group by modelo order by quantidade desc'
}
